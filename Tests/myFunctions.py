...
def  set_weights_old(y_data, option='balanced'):
...
def set_weights(y_data, option='balanced'):
    ...
    cw = compute_class_weight(class_weight=option, classes=classes, y=y_data)
    w = {i:j for i,j in zip(classes, cw)}
    return w
...
def MyML_DataLeakege(sFile, summaryFile, boxplotFile, nSel=0, nPCA=0, outVar='Class',nfold=3, seed=74):
    # nSel = 0 -> no Feature selection
    # df correction, preprocessing, scale, feature selection, PCA, SMOTE, ML
    ...
    # NEW TESTS ---- SHUFFLE Ydata --------------
    Ydata_shuffled = Ydata.copy()

    # Shuffle the array in place
    np.random.shuffle(Ydata_shuffled)
    
    Ydata = Ydata_shuffled.copy()
...
def custom_cross_val_score(clf, X, y, cv, seed=74):
    # training a model with CV and SMOTE inside each split
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        score = roc_auc_score(y_test_resampled, y_scores)
        scores.append(score)
    
    return np.array(scores)
    
def MLOuterCV_SMOTEafterSplit(Xdata, Ydata, label = 'my', class_weights = {0: 1, 1: 1}, folds = 3, seed = 74):
    # inputs:
    # data for X, Y; a label about data, number of folds, seeed
    ...
    for name, clf in zip(names, classifiers):
        ...
        # Evaluate pipeline with custom cross-validation and SMOTE
        scores = custom_cross_val_score(clf, Xdata, Ydata, cv=folds, seed=seed)
...
def MyML_SMOTEafterSplit(sFile, summaryFile, boxplotFile, nSel=0, nPCA=0, outVar='Class',nfold=3, seed=74):
    # nSel = 0 -> no Feature selection
    # df correction, preprocessing, scale, feature selection, PCA, SMOTE, ML
    ...
    df_results = None # all results 
    df_fold = MLOuterCV_SMOTEafterSplit(Xdata, Ydata, label = outVar, class_weights = class_weights, folds = nfold, seed = seed)
    df_fold['Dataset'] = outVar
    df_fold['folds'] = nfold
...
def UndersampleDF(df, sFile, seed=0):
    # Undersample balanced df
    Xdata, Ydata, Features = getDataFromDataFrame(df)  # out var = Class
    print('\n-> Dataframe undersampling balancing')
    print('Initial dimensions:', df.shape)
    
    # Use RandomUnderSampler instead of SMOTE
    undersampler = RandomUnderSampler(sampling_strategy='majority', random_state=seed)
    X_us, y_us = undersampler.fit_resample(Xdata, Ydata)
    
    # Create the resulting dataframe
    df_balanced = pd.DataFrame(X_us, columns=Features)
    df_balanced['Class'] = y_us  # add class column
    print('Final shape:', df_balanced.shape)
    
    # Save undersampled balanced dataset
    balancedFile = sFile[:-4] + '.ds_bal_under.csv'
    print('* Save balanced dataset:', balancedFile)
    
    df_balanced.to_csv(balancedFile, index=False)
    print('Done!')
    return df_balanced
  
def MyML_UnderBeforeSplit(sFile, summaryFile, boxplotFile, nSel=0, nPCA=0, outVar='Class',nfold=3, seed=74):
    # nSel = 0 -> no Feature selection
    # df correction, preprocessing, scale, feature selection, PCA, SMOTE, ML
    ...
    # Balancing dataframe using Undersampling
    df = UndersampleDF(df,sFile,seed)
